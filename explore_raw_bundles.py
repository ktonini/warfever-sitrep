#!/usr/bin/env python3
"""
The Last War Raw Bundle Explorer

Explores .raw files and bundle fragments to find images.
"""
...
if not assetbundles_path.exists():
    print(f"Error: AssetBundles directory not found")
    sys.exit(1)

print(f"Scanning raw bundles in: {assetbundles_path}")
...
# Get all .raw files and .bytes files
bundle_files = list(assetbundles_path.glob('*.raw'))
bundle_files.extend(assetbundles_path.glob('*.bytes'))
...
output_file = "raw_bundle_catalog.txt"
print(f"\n{'='*80}")
print(f"Saving complete catalog to: {output_file}")

with open(output_file, 'w', encoding='utf-8') as f:
    f.write("THE LAST WAR - RAW BUNDLE IMAGE CATALOG\n")
    f.write("="*80 + "\n\n")
